...
└── interactive_sine/
    ...
    └── app/
        ...
        ├── app_interactive_sine.py   ← ✏️  ENTRY POINT  (6 lines to edit per new app — see "❶ EDIT" block)
        ├── interactive_sine.env      ← ✏️  ENV CONFIG   (set LOGS_PATH and extras here; optional, "" to skip)
        ├── interactive_sine.py       ← ✏️  YOUR PAYLOAD (UI + logic — must define `render_run_page(sb)`)
        │
        ├── sandbox.py         ← 🔒  framework — do not edit
        ├── startup_interactive_sine.py ← 🔒  framework — do not edit (customize welcome/about text only)
...
entry point (`app_interactive_sine.py`) dynamically imports your payload module and renders it
inside a `▶ Run` tab, alongside `🏠 Startup` and `📋 Logs` tabs.

## Payload contract

Your payload module (`interactive_sine.py`, or your renamed equivalent) **must** define:
...
If the payload doesn't define `render_run_page(sb)`, `app_interactive_sine.py` raises:
```
...
In `app_interactive_sine.py`:

1. `_PAYLOAD_MODULE` – name of your feature module (no `.py`), e.g. `"interactive_sine"`
...
5. `_ENV_FILENAME` – name of the `.env` file next to `app_interactive_sine.py` (`""` to skip)
6. `_STANDALONE_PORT` – port used only by `python app_interactive_sine.py` direct execution

## To test (standalone)

```bash
python apps/interactive_sine/app/app_interactive_sine.py
```
...
The standalone `@ui.page("/")` handler at the bottom of `app_interactive_sine.py` (guarded by
